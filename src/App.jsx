...
const QUICK_PROMPTS = [
  ...
  'Give me 5 extra practice questions',
...
function Chatbot({ context }) {
  ...
  const send = async (text) => {
    ...
    setLoading(true);
    try {
      const reply = await sendChatMessage(next.filter(m => m.role !== 'system'), context);
  ...
  return (
    <>
      ...
      {open && (
        <div className="slide-up" style={{ position:'fixed', bottom:114, right:18, width:360, maxHeight:'68vh', background:'#13161d', border:'1px solid #252a36', borderRadius:16, display:'flex', flexDirection:'column', zIndex:200, overflow:'hidden', boxShadow:'0 8px 40px rgba(0,0,0,.6)' }}>
          <div style={{ padding:'13px 17px', borderBottom:'1px solid #252a36', display:'flex', alignItems:'center', justifyContent:'space-between', background:'#1a1e27' }}>
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          {context?.chapterTitle && (
            <div style={{ padding:'5px 14px', background:'rgba(79,156,249,0.05)', borderBottom:'1px solid #252a36' }}>
              <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:9, color:'#4f9cf9', letterSpacing:1 }}>STUDYING: </span>
              <span style={{ fontSize:11, color:'#8892a4' }}>{context.chapterTitle}</span>
            </div>
          )}
          ...
          <div style={{ flex:1, overflowY:'auto', padding:'12px 12px 6px' }}>
            ...
            {loading && (
              <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10 }}>
                <div style={{ width:24, height:24, borderRadius:'50%', background:'linear-gradient(135deg,#4f9cf9,#7f5ff9)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:12 }}>🤖</div>
                <div style={{ background:'#252a36', borderRadius:'14px 14px 14px 4px', padding:'10px 15px', display:'flex', gap:5, alignItems:'center' }}>
                  {[0,1,2].map(i => <div key={i} style={{ width:6, height:6, borderRadius:'50%', background:'#4f9cf9', animation:`blink 1.2s ease ${i*.2}s infinite` }}/>)}
                </div>
              </div>
            )}
            <div ref={bottomRef}/>
          ...
          {messages.length <= 1 && (
            <div style={{ padding:'4px 12px 8px', display:'flex', gap:6, flexWrap:'wrap' }}>
              {QUICK_PROMPTS.map((p,i) => (
                <button key={i} onClick={() => send(p)} style={{ background:'rgba(79,156,249,0.08)', border:'1px solid #4f9cf930', borderRadius:20, color:'#4f9cf9', cursor:'pointer', padding:'4px 10px', fontSize:11 }}>{p}</button>
              ))}
            </div>
          )}
      ...
      <div className="fade" style={{ width:'100%', maxWidth:400 }}>
        ...
        <div style={{ textAlign:'center', marginTop:20 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, marginBottom:4 }}>
            <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:9, color:'#3a3f4f', letterSpacing:1 }}>© {COPYRIGHT_YEAR} · OWNED BY</span>
            <span style={{ fontFamily:"'DM Serif Display',serif", fontSize:13, color:'#f9a84f' }}>Yination</span>
...
function CourseView({ course, user, progress, onBack, onProgressUpdate }) {
  ...
  const pct     = totalQ===0?0:Math.round(cp.openedQs.length/totalQ*100);
  ...
  return(
    <div>
      <div style={{background:'rgba(249,168,79,0.06)',border:'1px solid #f9a84f30',borderRadius:10,padding:'12px 16px',marginBottom:18,display:'flex',gap:10}}>
        ...
        <div><div style={{color:'#f9a84f',fontSize:13,fontWeight:600,marginBottom:3}}>Superuser Exclusive</div><div style={{color:'#8892a4',fontSize:12}}>Only you can promote or demote admins.</div></div>
      </div>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:10}}>
        {filtered.map((u,i)=>{const isAdm=admins.includes(u.username.toLowerCase());return(<div key={i} style={{background:'#13161d',border:`1px solid ${isAdm?'#da7ff030':'#252a36'}`,borderRadius:10,padding:'13px 17px',display:'flex',alignItems:'center',gap:13,flexWrap:'wrap'}}><Avatar name={u.display_name||u.username}/><div style={{flex:1,minWidth:140}}><div style={{fontSize:14,color:'#fff',fontWeight:500}}>{u.display_name||u.username}</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:'#8892a4',marginTop:2}}>@{u.username} · Yr {u.year}</div></div><RoleBadge role={isAdm?ROLE.ADMIN:ROLE.USER}/><button onClick={()=>toggleAdmin(u)} disabled={busy===u.username} style={{background:isAdm?'rgba(240,80,80,0.1)':'rgba(218,127,240,0.1)',border:`1px solid ${isAdm?'#f0505040':'#da7ff040'}`,borderRadius:7,color:isAdm?'#f05050':'#da7ff0',cursor:'pointer',padding:'6px 14px',fontSize:11,fontFamily:"'IBM Plex Mono',monospace",fontWeight:600}}>{busy===u.username?'…':isAdm?'Demote':'→ Make Admin'}</button></div>);})}
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.92)',zIndex:2000,overflow:'auto'}}>
      <div style={{maxWidth:880,margin:'0 auto',padding:'34px 20px 90px'}}>
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:26,flexWrap:'wrap',gap:14}}>
          <div><div style={{display:'flex',alignItems:'center',gap:10,marginBottom:4}}><Mono color={isSU2?'#f9a84f':'#da7ff0'} size={9}>{isSU2?'SUPERUSER PANEL':'ADMIN PANEL'}</Mono><RoleBadge role={user.role}/></div><h2 style={{fontFamily:"'DM Serif Display',serif",fontSize:24,color:'#fff'}}>Manage StudyHub</h2></div>
          <button onClick={onClose} style={{background:'none',border:'1px solid #252a36',borderRadius:8,color:'#8892a4',cursor:'pointer',padding:'9px 18px',fontSize:13}}>← Back</button>
        </div>
        ...
        <div style={{display:'flex',gap:4,borderBottom:'1px solid #252a36',marginBottom:22}}>
          {pTabs.map(t=>(<button key={t.id} onClick={()=>setTab(t.id)} style={{background:tab===t.id?'rgba(249,168,79,0.06)':'none',border:'none',borderBottom:tab===t.id?'2px solid #f9a84f':'2px solid transparent',color:tab===t.id?'#f9a84f':'#8892a4',cursor:'pointer',padding:'9px 16px',fontSize:13,fontWeight:tab===t.id?600:400}}>{t.label}</button>))}
        </div>
        {tab==='courses'&&(
          <div>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap',gap:10}}>
              <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                {[{label:'All',y:0},...YEARS.map(y=>({label:`Yr ${y}`,y}))].map(({label,y})=>(<button key={y} onClick={()=>setFilterY(y)} style={{background:filterY===y?(y===0?'rgba(136,146,164,0.1)':YEAR_BG[y]):'none',border:`1px solid ${filterY===y?(y===0?'#8892a4':YEAR_COLORS[y])+'60':'#252a36'}`,borderRadius:20,color:filterY===y?(y===0?'#8892a4':YEAR_COLORS[y]):'#8892a4',cursor:'pointer',padding:'5px 14px',fontSize:12,fontWeight:filterY===y?600:400}}>{label}</button>))}
              </div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:9}}>
              ...
              {filtered.map(c=>(<div key={c.id} style={{background:'#13161d',border:'1px solid #252a36',borderRadius:10,padding:'13px 17px',display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}><div style={{background:YEAR_BG[c.year],border:`1px solid ${YEAR_COLORS[c.year]}40`,borderRadius:5,padding:'3px 9px'}}><Mono color={YEAR_COLORS[c.year]} size={9}>Yr {c.year}</Mono></div><div style={{flex:1,minWidth:180}}><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:'#8892a4'}}>{c.courseName}</div><div style={{fontSize:14,color:'#fff',marginTop:2}}>{c.chapterTitle}</div></div><div style={{display:'flex',gap:6}}><Tag color="#4f9cf9">{c.conceptCount} concepts</Tag><Tag color="#7fda96">{c.qCount} questions</Tag></div><button onClick={()=>doDelete(c.id)} style={{background:'rgba(240,80,80,0.1)',border:'1px solid #f0505040',borderRadius:6,color:'#f05050',cursor:'pointer',padding:'5px 12px',fontSize:11,fontFamily:"'IBM Plex Mono',monospace",flexShrink:0}}>✕ Delete</button></div>))}
            </div>
          </div>
        )}
        {tab==='users'&&(
          <div style={{display:'flex',flexDirection:'column',gap:9}}>
            ...
            <div style={{marginTop:8,display:'flex',flexDirection:'column',gap:9}}>
              {allUsers.map((u,i)=>{const isAdm=admins.includes(u.username.toLowerCase());return(<div key={i} style={{background:'#13161d',border:'1px solid #252a36',borderRadius:10,padding:'12px 16px',display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}><Avatar name={u.display_name||u.username}/><div style={{flex:1}}><div style={{fontSize:14,color:'#fff',fontWeight:500}}>{u.display_name||u.username}</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:'#8892a4',marginTop:2}}>@{u.username} · Yr {u.year} · {new Date(u.created_at).toLocaleDateString()}</div></div><RoleBadge role={isAdm?ROLE.ADMIN:ROLE.USER}/><div style={{background:YEAR_BG[u.year],border:`1px solid ${YEAR_COLORS[u.year]}40`,borderRadius:5,padding:'3px 9px'}}><Mono color={YEAR_COLORS[u.year]} size={9}>Yr {u.year}</Mono></div></div>);})}
    ...
    <div style={{maxWidth:980,margin:'0 auto',padding:'34px 20px 80px'}}>
      ...
      <div style={{marginBottom:26}}>
        ...
        <div style={{display:'flex',gap:10,flexWrap:'wrap',marginTop:10}}>
          {YEARS.map(y=>{const active=activeYear===y;const st=yearStat(y);return<button key={y} onClick={()=>setActiveYear(y)} style={{background:active?YEAR_BG[y]:'#13161d',border:`1px solid ${active?YEAR_COLORS[y]+'60':'#252a36'}`,borderRadius:10,cursor:'pointer',padding:'10px 18px',transition:'all .15s',textAlign:'left'}}><div style={{fontFamily:"'DM Serif Display',serif",fontSize:16,color:active?YEAR_COLORS[y]:'#fff'}}>Year {y}</div>{st&&<div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:active?YEAR_COLORS[y]+'aa':'#8892a4',marginTop:2}}>{st}</div>}</button>;})}
        </div>
      </div>
      ...
      {visible.length===0?(
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(278px,1fr))',gap:14,marginTop:16}}>
          {visible.map((c,i)=>{const accent=YEAR_COLORS[c.year]||CARD_ACCENTS[i%CARD_ACCENTS.length];const p=pct(c.id);const viewed=progress[c.id]?.viewed;return(
            <div key={c.id} onClick={()=>onSelectCourse(c.id)} style={{background:'#1a1e27',border:'1px solid #252a36',borderRadius:12,padding:'18px 20px',cursor:'pointer',transition:'transform .15s',borderTop:`3px solid ${accent}`,position:'relative'}}
              ...
              <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:8}}><Tag color={accent}>{c.conceptCount} concepts</Tag><Tag color={accent}>{c.termCount} terms</Tag><Tag color={accent}>{c.qCount} Q&A</Tag></div>
...
export default function App() {
  ...
  const handleLogin=async u=>{setUser(u);if(u.role===ROLE.USER){const p=await dbLoadProgress(u.username);setProgress(p);}setView('home');};
  const handleLogout=()=>{setUser(null);setProgress({});setActive(null);setView('auth');};
  const handleSelect=async id=>{setLoading(true);const data=await dbLoadCourseData(id);const meta=courses.find(c=>c.id===id);if(data){setActive({id,data,year:meta?.year});setView('course');}setLoading(false);};
  const handleProgress=async p=>{setProgress(p);if(user?.role===ROLE.USER)await dbSaveProgress(user.username,p);};
