...
async function processPDF(base64Data, filename) {
  const res = await fetch('/api/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ base64Data, filename }) });
...
function InstallPrompt() {
  ...
  return (
    <div className="slide-down" style={{
      ...
      background: '#1a1e27', border: '1px solid #4f9cf940',
      borderRadius: 12, padding: '12px 18px',
      display: 'flex', alignItems: 'center', gap: 12,
      zIndex: 500, boxShadow: '0 4px 24px rgba(0,0,0,.5)',
...
const CopyrightBar = () => (
  <div style={{
    ...
    background: '#0d0f14', borderTop: '1px solid #1a1e27',
    padding: '8px 20px', display: 'flex', alignItems: 'center',
    justifyContent: 'space-between', zIndex: 100, flexWrap: 'wrap', gap: 6
  }}>
    {/* Left — brand + version */}
...
function Chatbot({ context }) {
    ...
    if (open && messages.length === 0) {
      setMessages([{
        ...
        content: context?.chapterTitle
          ? `Hi! I'm StudyBot. I can see you're studying "${context.chapterTitle}". Ask me anything — I can explain concepts, give extra examples, or generate more practice questions.`
  ...
  return (
    <>
      {/* Floating button */}
      <button onClick={() => setOpen(o => !o)} title="StudyBot — AI Tutor" style={{
        position: 'fixed', bottom: 52, right: 22, width: 52, height: 52,
        borderRadius: '50%', border: 'none', cursor: 'pointer', zIndex: 200,
        background: 'linear-gradient(135deg,#4f9cf9,#7f5ff9)',
        boxShadow: open ? 'none' : '0 4px 24px rgba(79,156,249,0.4)',
      ...
      {open && (
        <div className="slide-up" style={{
          position: 'fixed', bottom: 114, right: 18, width: 360,
          maxHeight: '68vh', background: '#13161d',
          border: '1px solid #252a36', borderRadius: 16,
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px 12px 6px' }}>
            {messages.map((m, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start', marginBottom: 10 }}>
                ...
                <div style={{
                  maxWidth: '78%',
                  background: m.role === 'user' ? 'linear-gradient(135deg,#4f9cf9,#7f5ff9)' : '#252a36',
                  color: m.role === 'user' ? '#fff' : '#e2e6f0',
            ...
            {loading && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                ...
                <div style={{ background: '#252a36', borderRadius: '14px 14px 14px 4px', padding: '10px 15px', display: 'flex', gap: 5, alignItems: 'center' }}>
                  {[0,1,2].map(i => <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: '#4f9cf9', animation: `blink 1.2s ease ${i * 0.2}s infinite` }} />)}
                </div>
              </div>
            )}
            <div ref={bottomRef} />
          ...
          <div style={{ padding: '9px 11px', borderTop: '1px solid #252a36', display: 'flex', gap: 8, alignItems: 'flex-end' }}>
            <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)}
              ...
              rows={1} style={{ flex: 1, background: '#0d0f14', border: '1px solid #252a36', borderRadius: 10, padding: '8px 11px', color: '#fff', fontSize: 13, fontFamily: "'DM Sans',sans-serif", resize: 'none', maxHeight: 80, lineHeight: 1.5 }} />
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.88)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1500, padding: 20, overflowY: 'auto' }}>
      <div className="fade" style={{ background: '#13161d', border: '1px solid #252a36', borderRadius: 16, padding: '28px 32px', maxWidth: 500, width: '100%', margin: 'auto' }}>
        ...
        {mode === 'ai' && (
          <>
            <p style={{ color: '#8892a4', fontSize: 12.5, marginBottom: 12 }}>Upload a PDF — Gemini reads it and generates concepts, definitions, mechanisms &amp; <strong style={{ color: '#f9a84f' }}>25 practice questions</strong>.</p>
        ...
        {status === 'processing' && <div style={{ background: 'rgba(79,156,249,0.08)', border: '1px solid #4f9cf930', borderRadius: 8, padding: '10px 14px', color: '#4f9cf9', fontSize: 13, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 10 }}><span style={{ animation: 'spin 1s linear infinite', display: 'inline-block' }}>⟳</span>{mode === 'ai' ? 'Gemini is processing your PDF…' : 'Saving course…'}</div>}
...
function CourseView({ course, user, progress, onBack, onProgressUpdate }) {
  ...
  const chatContext = {
    courseName: d.courseName, chapterTitle: d.chapterTitle,
    summary: [d.keyConcepts?.slice(0, 5).map(c => c.title).join(', '), d.chapters?.map(c => c.name).join(', ')].filter(Boolean).join(' | ')
  };
          ...
          return (
            <div key={i} style={{ background: '#13161d', border: `1px solid ${isAdm ? '#da7ff030' : '#252a36'}`, borderRadius: 10, padding: '13px 17px', display: 'flex', alignItems: 'center', gap: 13, flexWrap: 'wrap' }}>
              ...
              <div style={{ flex: 1, minWidth: 140 }}>
                ...
                <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, color: '#8892a4', marginTop: 2 }}>@{u.username} · Yr {u.year}</div>
              </div>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.92)', zIndex: 2000, overflow: 'auto' }}>
      <div style={{ maxWidth: 880, margin: '0 auto', padding: '34px 20px 90px' }}>
        ...
        <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid #252a36', marginBottom: 22 }}>
          {panelTabs.map(t => (<button key={t.id} onClick={() => setTab(t.id)} style={{ background: tab === t.id ? 'rgba(249,168,79,0.06)' : 'none', border: 'none', borderBottom: tab === t.id ? '2px solid #f9a84f' : '2px solid transparent', color: tab === t.id ? '#f9a84f' : '#8892a4', cursor: 'pointer', padding: '9px 16px', fontSize: 13, fontWeight: tab === t.id ? 600 : 400 }}>{t.label}</button>))}
        </div>
        {tab === 'courses' && (
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
              ...
              {filtered.map(c => (
                <div key={c.id} style={{ background: '#13161d', border: '1px solid #252a36', borderRadius: 10, padding: '13px 17px', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 180 }}><div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, color: '#8892a4' }}>{c.courseName}</div><div style={{ fontSize: 14, color: '#fff', marginTop: 2 }}>{c.chapterTitle}</div></div>
              ...
              {allUsers.map((u, i) => {
                ...
                return (
                  <div key={i} style={{ background: '#13161d', border: '1px solid #252a36', borderRadius: 10, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                    <Avatar name={u.display_name || u.username} />
                    <div style={{ flex: 1 }}><div style={{ fontSize: 14, color: '#fff', fontWeight: 500 }}>{u.display_name || u.username}</div><div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, color: '#8892a4', marginTop: 2 }}>@{u.username} · Yr {u.year} · {new Date(u.created_at).toLocaleDateString()}</div></div>
                    <RoleBadge role={isAdm ? ROLE.ADMIN : ROLE.USER} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 3 }}>
              ...
              {user.role === ROLE.USER && <Mono color="#8892a4" size={9}>Year {user.year} · @{user.username}</Mono>}
            </div>
          </div>
        </div>
